...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .pdv-header{padding:1.5rem;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--primary);background:var(--primary);color:#fff;}
        ...
        .pdv-header-total{background:rgba(255,255,255,.1) !important;border-color:rgba(255,255,255,.25) !important;color:#fff !important;}
        .pdv-header-total span{color:#fff !important;}
        .pdv-produtos{flex:1;overflow-y:auto;padding:1.5rem;background:var(--bg-pdv);}
        .produto-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:1rem;}
        .produto-card{background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:.85rem .85rem 1rem;cursor:pointer;transition:.2s;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px;min-height:280px;}
        .produto-card:hover{border-color:var(--primary);transform:translateY(-2px);box-shadow:0 4px 12px rgba(0,0,0,.05);}
        ...
        .pdv-right-header{padding:1.15rem 1.35rem;border-bottom:1px solid rgba(255,255,255,.18);background:linear-gradient(135deg,#00349a 0%,#0b48bd 58%,#0f62d8 100%);color:#fff;display:flex;align-items:center;justify-content:space-between;gap:1rem;box-shadow:0 16px 34px -30px rgba(0,20,70,.7);}
        ...
        .btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:12px 20px;border-radius:8px;font-weight:600;cursor:pointer;border:none;font-family:inherit;font-size:.85rem;transition:.2s;text-decoration:none;}
        .btn-primary{background:var(--primary);color:#fff !important;}
        .btn-success{background:var(--success);color:white !important;}
        .form-control{width:100%;padding:12px 16px;border-radius:8px;border:1px solid var(--border);font-family:inherit;font-size:.9rem;background:var(--bg-card);color:inherit;}
        input:-webkit-autofill { -webkit-box-shadow: 0 0 0 30px var(--bg-card) inset !important; -webkit-text-fill-color: inherit !important; }
        .pdv-footer{padding:1rem 1.15rem 1.15rem;border-top:1px solid rgba(255,255,255,.16);background:linear-gradient(135deg,#00349a 0%,#093fa7 100%);color:#fff;box-shadow:0 -18px 42px -34px rgba(0,20,70,.9);}
        ...
        .checkout-actions .btn{width:100%;min-height:56px;border-radius:14px;font-size:.92rem;font-weight:900;box-shadow:0 16px 30px -24px rgba(0,0,0,.5);}
        .checkout-actions .btn i{width:20px;height:20px;}
        .checkout-actions .btn-success{background:linear-gradient(135deg,#10b981,#0f9f91);color:#fff !important;border:1px solid rgba(255,255,255,.18);box-shadow:0 16px 28px -22px rgba(16,185,129,.85);}
        .checkout-actions .btn-success:disabled{opacity:.62;cursor:not-allowed;filter:saturate(.75);box-shadow:none;}
        .checkout-actions .btn-clear{background:#fff !important;color:var(--danger) !important;border:1px solid rgba(255,255,255,.45) !important;}
        ...
        .pdv-dialog-actions .btn{width:100%;min-height:46px;border-radius:12px;font-size:.9rem;}
        .pdv-dialog-cancel{background:#f8fafc;color:#0f172a;border:1px solid var(--border);}
        .pdv-dialog-confirm{background:var(--primary);color:#fff !important;}
        .pdv-dialog-danger{background:var(--danger);color:#fff !important;}
        ...
        input[type=number]{padding:8px;border:1px solid var(--border);border-radius:6px;background:var(--bg-pdv);color:inherit;}
        .search-bar{padding:0 1.5rem 1rem;border-bottom:1px solid var(--border);background:var(--bg-pdv);}
...
<div class="pdv-layout" id="pdv-layout">
    ...
    <div class="pdv-left" id="pdv-left">
        ...
        <!-- Busca cliente -->
        <div style="padding:1rem 1.5rem;background:var(--bg-pdv);border-bottom:1px solid var(--border);display:flex;gap:.75rem;flex-wrap:wrap;">
        ...
        <div class="pdv-footer">
            ...
            <div id="forma-pagamento-block" class="payment-block">
                ...
                <select id="forma-pagamento" class="form-control" onchange="calcularTotal()">
                    <option>Pix</option><option>Dinheiro</option><option>Cartão de Débito</option><option>Cartão de Crédito</option><option>Saldo Mercado Livre</option>
                </select>
            </div>
...
function abrirDialogoPdv(opcoes) {
    ...
    confirm.className = 'btn ' + (config.danger ? 'pdv-dialog-danger' : 'pdv-dialog-confirm');
...
function calcularTotal() {
    ...
    if (forma.includes('cr') || forma.includes('credito') || forma.includes('crÃ©dito')) {
        taxaPercentual = taxasCartao.credito;
    } else if (forma.includes('debito') || forma.includes('dÃ©bito') || forma.includes('débito')) {
        taxaPercentual = taxasCartao.debito;
    }
