...
$isCancelled = $statusAtualLabel === 'Cancelado';
...
<?php if (($_GET['cancelled'] ?? '') === '1'): ?>
    <div class="alert-success"><i data-lucide="check-circle"></i> OS cancelada logicamente. Todos os dados foram preservados.</div>
<?php elseif (!empty($_GET['cancel_error'])): ?>
    <div class="alert-success" style="background:#fee2e2;border-color:#fecaca;color:#991b1b;">
        <i data-lucide="alert-triangle"></i> <?= ($_GET['cancel_error'] ?? '') === 'CANCELLATION_BLOCKED_MOVEMENTS' ? 'Esta OS possui movimentacoes e nao pode ser cancelada sem estorno validado.' : 'Nao foi possivel cancelar esta OS.' ?>
    </div>
<?php elseif (($_GET['error'] ?? '') === 'cancelled_read_only'): ?>
    <div class="alert-success" style="background:#fee2e2;border-color:#fecaca;color:#991b1b;"><i data-lucide="lock"></i> OS cancelada e disponivel somente para consulta e impressao.</div>
<?php endif; ?>

<?php if ($isCancelled): ?><div class="alert-success" style="background:#fee2e2;border-color:#dc2626;color:#991b1b;font-weight:800;"><i data-lucide="x-circle"></i> CANCELADA — operacoes bloqueadas; dados preservados.</div><?php endif; ?>
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
    ...
    <div style="display:flex;gap:.75rem;flex-wrap:wrap;">
        <?php 
        $linkConsulta = app_url('index.php?url=consulta&os=' . rawurlencode((string)$os['numero_os']));
        ...
        $wpp_msg .= "- *Acompanhe em tempo real:* {$linkConsulta}\n";
        ...
        <a href="<?= e(route_url('consulta', ['os' => $os['numero_os']])) ?>" target="_blank" class="btn btn-secondary" title="Página pública de acompanhamento da OS">
            <i data-lucide="external-link"></i> Link do Cliente
        </a>
        <?php if (!$isCancelled): ?><a href="<?= e(route_url('os/edit', ['id' => (int) $os['id']])) ?>" class="btn btn-secondary">
        ...
        <?php if (current_user_profile() === 'Administrador' && !$isCancelled): ?>
            <form method="POST" action="<?= e(route_url('os/cancel')) ?>" data-confirm="Cancelar logicamente esta OS? Todos os dados serao preservados." onclick="event.stopPropagation()">
                <?= csrf_field() ?>
                <input type="hidden" name="id" value="<?= (int) $os['id'] ?>">
                <input type="text" name="motivo" class="form-control" required maxlength="500" placeholder="Motivo obrigatorio" style="min-width:220px;">
                <button type="submit" class="btn" style="background:var(--danger);color:#fff;" onclick="event.stopPropagation()">
                    <i data-lucide="x-circle"></i> Cancelar OS
                </button>
            </form>
        <?php endif; ?>
    </div>
