                    ...
                    <?php foreach ($ordens as $os): ?>
                        ...
                        <tr style="cursor:pointer;" onclick="window.location='<?= e($viewUrl) ?>'">
                            ...
                            <td style="text-align:right;" onclick="event.stopPropagation()">
                                ...
                                <?php if ($statusLabel !== 'Cancelado'): ?><a href="<?= e($editUrl) ?>" class="btn" style="padding:6px;color:var(--warning);background:none;"><i data-lucide="edit-3" style="width:16px;"></i></a><?php endif; ?>
                                <?php if ($whatsappUrl !== ''): ?>
                                ...
                                <?php endif; ?>
                                <?php if (current_user_profile() === 'Administrador' && $statusLabel !== 'Cancelado'): ?>
                                    <form method="POST" action="<?= e(route_url('os/cancel')) ?>" style="display:inline-flex;" onclick="event.stopPropagation()" onsubmit="const motivo=window.prompt('Informe o motivo do cancelamento da OS #<?= e($numeroOs) ?>:');if(!motivo||!motivo.trim())return false;this.elements.motivo.value=motivo.trim();">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="id" value="<?= $osId ?>">
                                        <input type="hidden" name="motivo" value="">
                                        <button type="submit" class="btn" style="padding:6px;color:var(--danger);background:none;" title="Cancelar OS" aria-label="Cancelar OS" onclick="event.stopPropagation()"><i data-lucide="x-circle" style="width:16px;"></i></button>
                                    </form>
                                <?php endif; ?>
                            </td>
                        </tr>
