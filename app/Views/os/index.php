            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($ordens as $os): ?>
                        ...
                        $statusBadge = $statusColors[$status] ?? 'badge-gray';
                        ...
                        $wppMsg = "{$emojiRocket} *OS #{$numeroOs}* - *CONECTADOS*\n" .
                            "{$emojiPhone} *Aparelho:* {$aparelho}\n" .
                        ...
                        <tr style="cursor:pointer;" onclick="window.location='<?= e($viewUrl) ?>'">
                            ...
                            <td style="font-size:.85rem;">
                                ...
                                <?php if (current_user_profile() === 'Administrador'): ?>
                                    <form method="POST" action="<?= e(route_url('os/delete')) ?>" data-confirm="Excluir a OS #<?= e($numeroOs) ?>? Esta acao remove a ordem, historico, pagamentos e fotos anexadas.">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="id" value="<?= $osId ?>">
                                        <button type="submit" class="btn" style="padding:6px;color:var(--danger);background:none;" title="Excluir OS" aria-label="Excluir OS"><i data-lucide="trash-2" style="width:16px;"></i></button>
                                    </form>
                                <?php endif; ?>
                            </td>
                            <td><span class="badge <?= e($statusBadge) ?>"><?= e($status) ?></span></td>
